try {
    ...
    const API = "http://my-bots-api-4b693c-3c2875-45-130-165-63.traefik.me/";
    ...
    async function instagram(query) {
        const trimmedQuery = query.trim();
        const url = `api/instagram-download?url=${encodeURIComponent(trimmedQuery)}`;
        const response = await axios.get(API + url, { responseType: "json" });
    ...
    async function tiktok(query) {
       const trimmedQuery = query.trim();
       const url = `api/tiktok-download?url=${encodeURIComponent(trimmedQuery)}`;
       const response = await axios.get(API + url, { responseType: "json" });
       const media = response.data.url;
       const videoResponse = await axios.get(media, { responseType: "arraybuffer" });
       return Buffer.from(videoResponse.data);
    ...
    async function spotify(query) {
        try {
            const trimmedQuery = query.trim();
            const url = `api/spotify-download?url=${encodeURIComponent(trimmedQuery)}`;
            const response = await axios.get(API + url, { responseType: "json" });
            
            if (response && response.data && response.data.link) {
                const media = response.data.link;
                const videoResponse = await axios.get(media, { responseType: "arraybuffer" });
                return Buffer.from(videoResponse.data);
            }
        } catch (error) {
